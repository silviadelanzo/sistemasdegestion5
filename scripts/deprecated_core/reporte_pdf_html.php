...
try {
    ...
    $sql_sin_cat = "SELECT 'Sin Categoría' as categoria,
                           codigo, nombre as producto, stock,
                           precio_venta, precio_compra,
                           (stock * precio_venta) as valor_total
                    FROM productos 
                    WHERE categoria_id IS NULL AND activo = 1
                    ORDER BY nombre";
    $sin_categoria = $pdo->query($sql_sin_cat)->fetchAll(PDO::FETCH_ASSOC);
...
<body>
    <div class="container">
        <div class="print-buttons">
            <h3>📄 Reporte de Inventario - Exportación PDF</h3>
            <p>Este reporte está optimizado para imprimir como PDF desde tu navegador</p>
                    ...
                    <tbody>
                        ...
                        foreach ($todos_productos as $index => $producto):
                            ...
                            if ($categoria_actual != '' && $categoria_actual != $producto['categoria']) {
                                ...
                                echo "<td colspan='3'>TOTAL " . htmlspecialchars($categoria_actual) . " ({$productos_categoria} productos)</td>";
                                echo "<td class='text-center'>" . number_format($total_categoria_stock) . "</td>";
                                echo "<td colspan='2'></td>";
                                echo "<td class='text-right'>" . formatCurrency($total_categoria_valor) . "</td>";
                                echo "</tr>";
                            ...
                            $total_categoria_valor += $producto['valor_total'];
                            $productos_categoria++;
                        ?>
                            <tr>
                        ...
                        <?php if ($categoria_actual != ''): ?>
                            <tr style='background-color: #e9ecef; font-weight: bold;'>
                                <td colspan='3'>TOTAL <?php echo htmlspecialchars($categoria_actual); ?> (<?php echo $productos_categoria; ?> productos)</td>
                                <td class='text-center'><?php echo number_format($total_categoria_stock); ?></td>
                                <td colspan='2'></td>
                                <td class='text-right'><?php echo formatCurrency($total_categoria_valor); ?></td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
