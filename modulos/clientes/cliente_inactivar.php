...
if ($cliente_id <= 0) {
    ...
    // Buscar cliente activo y no eliminado
    $stmt = $pdo->prepare("SELECT * FROM clientes WHERE id=? AND eliminado=0 AND activo=1");
    $stmt->execute([$cliente_id]);
    $cliente = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$cliente) {
        $error = "Cliente no encontrado, ya está inactivo o eliminado.";
    } else {
        ...
        if ($pedidos_pendientes > 0) {
            $error = "No se puede inactivar: el cliente tiene $pedidos_pendientes pedido(s) pendiente(s).";
        }
    }
}

// Procesar confirmación de inactivación
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error && $cliente) {
    try {
        $stmt = $pdo->prepare("UPDATE clientes SET activo=0, fecha_modificacion=NOW(), modificado_por=? WHERE id=?");
        $stmt->execute([$usuario_id, $cliente_id]);
        header("Location: clientes_inactivos.php?msg=inactivado");
        exit;
    } catch (Exception $e) {
        ...
        <?php elseif ($cliente): ?>
            <div class="card shadow">
                <div class="card-header bg-secondary text-white">
                    <h5 class="mb-0"><i class="bi bi-person-x"></i> Inactivar cliente</h5>
                </div>
                <div class="card-body">
                    <p>¿Está seguro que desea inactivar el siguiente cliente?</p>
                    <ul>
                    ...
                    <form method="post">
                        <button type="submit" class="btn btn-secondary">
                            <i class="bi bi-person-x"></i> Inactivar cliente
                        </button>
...
<!-- Mensaje en clientes_inactivos.php si fue inactivado -->
<?php if (isset($_GET['msg']) && $_GET['msg'] == 'inactivado'): ?>
<script>
    window.onload = function() {
        let alerta = document.createElement('div');
        alerta.className = 'alert alert-secondary alert-dismissible fade show position-fixed';
        alerta.style = 'top:70px;right:20px;z-index:1056;min-width:300px;';
        alerta.role = 'alert';
        alerta.innerHTML = '<i class="bi bi-person-x me-2"></i>Cliente inactivado correctamente.<button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
