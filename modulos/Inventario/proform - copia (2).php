...
if ($producto_id > 0) {
    ...
    if ($producto) {
        ...
        // Cargar proveedores (para mostrar selección previa en Paso 2)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'guardar_basico') {
        // Paso 1: guardar datos básicos en productos
        ...
        if (empty($codigo_barra)) $errores[] = 'El código de barras es obligatorio.';
        if ($precio_compra <= 0) $errores[] = 'El precio de compra debe ser mayor a 0.';

        // Validación de unicidad de código de barras en DB
        if (empty($errores)) {
            try {
                if ($es_edicion) {
        ...
        if (empty($errores)) {
            try {
                if ($es_edicion) {
                    $sql = "UPDATE productos SET codigo_barra=?, nombre=?, descripcion=?, categoria_id=?, lugar_id=?, unidad_medida=?, precio_compra=?, precio_minorista=?, precio_mayorista=?, precio_venta=?, stock=?, stock_minimo=?, stock_maximo=?, usar_control_stock=?, usar_alerta_vencimiento=?, fecha_vencimiento=?, dias_alerta_vencimiento=?, moneda_id=?, impuesto_id=? WHERE id=?";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([
                        $codigo_barra, $nombre, $descripcion, $categoria_id, $lugar_id, $unidad_medida,
                        $precio_compra, $precio_minorista, $precio_mayorista, $precio_venta,
                        ...
                        $moneda_id, $impuesto_id, $producto_id
                    ]);
                } else {
                    // INSERT productos (solo datos básicos)
                    $sql = "INSERT INTO productos (codigo, codigo_barra, nombre, descripcion, categoria_id, lugar_id, unidad_medida, precio_compra, precio_minorista, precio_mayorista, precio_venta, stock, stock_minimo, stock_maximo, usar_control_stock, usar_alerta_vencimiento, fecha_vencimiento, dias_alerta_vencimiento, moneda_id, impuesto_id, publicar_web)
                            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([
                ...
                // PRG: redirigir a paso proveedores
        ...
        // Si hubo errores, nos quedamos en el paso básico
        $step = 'basico';
    }

    if ($accion === 'guardar_proveedores') {
        // Paso 2: guardar proveedores, imagen y publicar_web
        if ($producto_id <= 0) {
            ...
            if (empty($errores)) {
                try {
                    // Normalizar proveedores a NULL si están vacíos o no numéricos
                    ...
                    // Actualizar publicar_web en productos
...
try {
    ...
    $impuestos = $pdo->query("SELECT id, nombre FROM impuestos ORDER BY nombre")->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container mt-5">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <h2 class="mb-0"><?php echo $es_edicion ? 'Editar producto' : 'Alta de producto'; ?></h2>
        <?php if ($es_edicion): ?>
            <span class="badge text-bg-secondary">Código: <?php echo htmlspecialchars($codigo); ?></span>
        <?php else: ?>
            <span class="badge text-bg-info">Próximo código: <?php echo htmlspecialchars($codigo); ?></span>
        <?php endif; ?>
    </div>
    ...
    <?php if ($step === 'basico'): ?>
        ...
        <form method="post" class="card p-4 bg-white">
            ...
            <table class="table table-bordered align-middle">
                    ...
                    <td>
                        <select name="impuesto_id" class="form-select">
                            <?php foreach ($impuestos as $imp): ?>
                                <option value="<?php echo $imp['id']; ?>" <?php if (($impuesto_id ?? '') == $imp['id']) echo 'selected'; ?>><?php echo htmlspecialchars($imp['nombre']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    <td><label class="form-label">Precio de Compra *</label></td>
                    <td><input type="number" name="precio_compra" class="form-control" step="0.01" min="0" required value="<?php echo htmlspecialchars($precio_compra ?? ''); ?>"></td>
                    <td><label class="form-label">Precio Minorista</label></td>
                    <td><input type="number" name="precio_minorista" class="form-control" step="0.01" min="0" value="<?php echo htmlspecialchars($precio_minorista ?? ''); ?>"></td>
                </tr>
                <tr>
                    <td><label class="form-label">Precio Mayorista</label></td>
                    <td><input type="number" name="precio_mayorista" class="form-control" step="0.01" min="0" value="<?php echo htmlspecialchars($precio_mayorista ?? ''); ?>"></td>
                    <td><label class="form-label">Precio de Venta</label></td>
                    <td><input type="number" name="precio_venta" class="form-control" step="0.01" min="0" value="<?php echo htmlspecialchars($precio_venta ?? ''); ?>"></td>
                </tr>
                <tr><td colspan="4"><b>Stock</b></td></tr>
                <tr>
                    <td><label class="form-label">Stock Actual</label></td>
                    <td><input type="number" name="stock" class="form-control" min="0" value="<?php echo htmlspecialchars($stock ?? ''); ?>"></td>
                    <td><label class="form-label">Stock Mínimo</label></td>
                    <td><input type="number" name="stock_minimo" class="form-control" min="0" value="<?php echo htmlspecialchars($stock_minimo ?? ''); ?>"></td>
                </tr>
                <tr>
                    <td><label class="form-label">Stock Máximo</label></td>
                    <td><input type="number" name="stock_maximo" class="form-control" min="0" value="<?php echo htmlspecialchars($stock_maximo ?? ''); ?>"></td>
                    <td colspan="2">
                ...
                <tr>
                    <td colspan="4">
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="usar_alerta_vencimiento"><b>Este producto usa vencimiento</b></label>
                        </div>
                    </td>
                </tr>
    ...
    <?php if ($step === 'proveedores'): ?>
        <!-- FORM PASO 2: Proveedores + imagen + publicar -->
        <?php if (!$es_edicion): ?>
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                <select name="proveedor_alternativo<?php echo str_pad($i,2,'0',STR_PAD_LEFT); ?>" class="form-select">
                                    ...
                                    <?php foreach ($proveedores_list as $prov): ?>
                                        <option value="<?php echo $prov['id']; ?>" <?php if ((${'proveedor_alternativo0'.$i} ?? '') == $prov['id']) echo 'selected'; ?>><?php echo htmlspecialchars($prov['razon_social']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </td>
                        </tr>
                    ...
                    <tr>
                        <td colspan="4">
                            <div class="form-check">
                                ...
                                <label class="form-check-label">Publicar en tienda web</label>
                            </div>
                        </td>
                    </tr>
