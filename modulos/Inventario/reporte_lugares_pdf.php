...
try {
    ...
    // Obtener datos del inventario por lugares
    $sql_totales = "SELECT 
    ...
    // Productos sin lugar
    $sql_sin_lugar = "SELECT 'Sin Ubicación' as lugar,
                             p.codigo, p.nombre as producto, p.stock,
                             p.precio_venta, p.precio_compra,
                             c.nombre as categoria,
                             (p.stock * p.precio_venta) as valor_total
                      FROM productos p
                      LEFT JOIN categorias c ON p.categoria_id = c.id
                      WHERE p.lugar_id IS NULL AND p.activo = 1
                      ORDER BY p.nombre";
    $sin_lugar = $pdo->query($sql_sin_lugar)->fetchAll(PDO::FETCH_ASSOC);

    // Combinar todos los productos
    $todos_productos = array_merge($productos, $sin_lugar);
} catch (Exception $e) {
...
<head>
    ...
    <title>Reporte de Inventario por Lugares - PDF</title>
    <style>
...
<body>
    <div class="container">
        <div class="print-buttons">
            <h3>📍 Reporte de Inventario por Lugares - Exportación PDF</h3>
            <p>Este reporte muestra la distribución de productos por ubicación física</p>
        ...
        <div class="report-header">
            <h1>📍 REPORTE DE INVENTARIO POR LUGARES</h1>
        ...
        <?php else: ?>
            ...
            <div class="totals-box">
                ...
                <div class="totals-grid">
                    <div class="total-item">
                        <div class="total-label">Lugares</div>
            ...
            <h2>📋 Detalle de Productos por Lugar</h2>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        foreach ($todos_productos as $index => $producto):
                            // Si cambia el lugar, mostrar totales del anterior
                            if ($lugar_actual != '' && $lugar_actual != $producto['lugar']) {
                                echo "<tr style='background-color: #e9ecef; font-weight: bold;'>";
                                echo "<td colspan='4'>TOTAL " . htmlspecialchars($lugar_actual) . " ({$productos_lugar} productos)</td>";
                                echo "<td class='text-center'>" . number_format($total_lugar_stock) . "</td>";
                                echo "<td></td>";
                                echo "<td class='text-right'>" . formatCurrency($total_lugar_valor) . "</td>";
                                echo "</tr>";
                            ...
                            $lugar_actual = $producto['lugar'];
                            $total_lugar_stock += $producto['stock'];
                            $total_lugar_valor += $producto['valor_total'];
                            $productos_lugar++;
                        ?>
                            <tr>
                                <td><?php echo htmlspecialchars($producto['lugar']); ?></td>
                        ...
                        <?php if ($lugar_actual != ''): ?>
                            <tr style='background-color: #e9ecef; font-weight: bold;'>
                                <td colspan='4'>TOTAL <?php echo htmlspecialchars($lugar_actual); ?> (<?php echo $productos_lugar; ?> productos)</td>
                                <td class='text-center'><?php echo number_format($total_lugar_stock); ?></td>
                                <td></td>
                                <td class='text-right'><?php echo formatCurrency($total_lugar_valor); ?></td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
