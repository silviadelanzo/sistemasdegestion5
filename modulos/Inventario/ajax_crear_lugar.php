...
try {
    $pdo = conectarDB();
    $stmt = $pdo->prepare("SELECT id FROM lugares WHERE nombre = ? AND cuenta_id = ?");
    $stmt->execute([$nombre, $cuenta_id]);
    if ($stmt->fetch()) {
        echo json_encode(['error' => 'El lugar ya existe en tu cuenta']);
        exit;
    }

    $stmt = $pdo->prepare("INSERT INTO lugares (cuenta_id, nombre) VALUES (?, ?)");
    ...
    echo json_encode(['error' => 'Error al crear lugar']);
}