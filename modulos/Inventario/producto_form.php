...
// Procesar formulario (todo en uno)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'guardar_todo') {
    ...
    if (empty($lugar_id)) $errores[] = 'Debes seleccionar un lugar.';
        ...
        try {
            ...
            if ($es_edicion) {
                $sql = "UPDATE productos SET
                    ...
                    moneda_id=?, impuesto_id=?, publicar_web=?
            ...
            // Guardar proveedores
            $proveedor_principal = intval($proveedor_principal);
            $proveedor_alternativo01 = !empty($proveedor_alternativo01) ? intval($proveedor_alternativo01) : null;
            $proveedor_alternativo02 = !empty($proveedor_alternativo02) ? intval($proveedor_alternativo02) : null;
            $proveedor_alternativo03 = !empty($proveedor_alternativo03) ? intval($proveedor_alternativo03) : null;
            $proveedor_alternativo04 = !empty($proveedor_alternativo04) ? intval($proveedor_alternativo04) : null;
            ...
            if (intval($check->fetchColumn()) > 0) {
                $sql_pp = "UPDATE productos_proveedores SET
                    proveedor_principal=?, proveedor_alternativo01=?, proveedor_alternativo02=?,
                    proveedor_alternativo03=?, proveedor_alternativo04=?
                    WHERE producto_id=?";
                $stmt_pp = $pdo->prepare($sql_pp);
                $stmt_pp->execute([
                    $proveedor_principal, $proveedor_alternativo01, $proveedor_alternativo02,
                    $proveedor_alternativo03, $proveedor_alternativo04, $producto_id
                ]);
            } else {
                $sql_pp = "INSERT INTO productos_proveedores
                    (producto_id, proveedor_principal, proveedor_alternativo01, proveedor_alternativo02, proveedor_alternativo03, proveedor_alternativo04)
                    VALUES (?,?,?,?,?,?)";
                $stmt_pp = $pdo->prepare($sql_pp);
                $stmt_pp->execute([
                    $producto_id, $proveedor_principal, $proveedor_alternativo01, $proveedor_alternativo02,
                    $proveedor_alternativo03, $proveedor_alternativo04
                ]);
            }
            ...
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
                ...
                $stmt->execute([$producto_id]);
                $codigo_img = $stmt->fetchColumn();
                $img_dir = __DIR__ . '/../../assets/img/productos/';
                if (!is_dir($img_dir)) @mkdir($img_dir, 0777, true);
                $destino = $img_dir . $codigo_img . '.jpg';
            ...
            $pdo->commit();
            registrar_auditoria('PRODUCTO_GUARDADO_COMPLETO', 'productos', $producto_id, "Producto guardado completamente: " . $nombre);
...
// Si se presiona "Cancelar", redirigir sin guardar
...
<head>
    <meta charset="UTF-8">
    <title><?php echo $es_edicion ? 'Editar Producto' : 'Nuevo Producto'; ?></title>
...
<div class="container">
    <div class="hero-blue">
        <h3><?php echo $es_edicion ? 'Editar Producto' : 'Nuevo Producto'; ?></h3>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered align-middle mb-0">
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <select name="categoria_id" class="form-select me-2" id="categoria_id" required>
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm" onclick="crearCategoria()">
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            <select name="lugar_id" class="form-select me-2" id="lugar_id" required>
                                <option value="">-- Seleccionar --</option>
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm" onclick="crearLugar()">
                                <i class="bi bi-plus"></i>
                            </button>
                        </div>
                    </td>
                    ...
                    <td>
                        <select id="impuesto_id" name="impuesto_id" class="form-select" required>
                            <?php foreach ($impuestos as $imp): ?>
                                <option value="<?php echo $imp['id']; ?>" data-porcentaje="<?php echo (float)$imp['porcentaje']; ?>" <?php if (($impuesto_id ?? '') == $imp['id']) echo 'selected'; ?>>
                ...
                <tr>
                    <td colspan="4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="usar_alerta_vencimiento" id="usar_alerta_vencimiento" value="1"
                                onchange="document.getElementById('vencimiento_fields').style.display = this.checked ? 'table-row' : 'none';"
                                <?php if (!empty($usar_alerta_vencimiento)) echo 'checked'; ?>>
                            <label class="form-check-label fw-bold">Este producto usa vencimiento</label>
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="4" class="text-center">
                        ...
                        <button type="submit" name="accion" value="cancelar" class="btn btn-secondary ms-2">Cancelar</button>
                    </td>
                </tr>
            </table>
        </div>
...
// === Crear categoría/lugar al vuelo ===
function crearCategoria() {
    const nombre = prompt("Nombre de la nueva categoría:");
    if (!nombre || nombre.trim() === '') return;
    fetch('ajax_crear_categoria.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'nombre=' + encodeURIComponent(nombre.trim())
    })
    .then(res => res.json())
    .then(data => {
        if (data.error) {
