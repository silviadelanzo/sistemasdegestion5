...
$producto = [
    'codigo_interno' => '', // Se generará automáticamente en el try/catch
    'codigo_producto' => '',
    'nombre' => '',
    'descripcion' => '',
    'categoria_id' => '',
...
try {
    ...
    // Generar código secuencial para productos nuevos
    if (!$es_edicion) {
        $stmt_ultimo = $pdo->query("SELECT codigo_interno FROM productos WHERE codigo_interno LIKE 'PROD-%' ORDER BY id DESC LIMIT 1");
        $ultimo_codigo = $stmt_ultimo->fetchColumn();
        ...
        $producto['codigo_interno'] = 'PROD-' . str_pad($nuevo_numero, 7, '0', STR_PAD_LEFT);
    }
...
<body>
        ...
        <div class="form-body">
            ...
            <form method="POST" enctype="multipart/form-data" id="formProducto">
                ...
                <ul class="nav nav-tabs nav-tabs-custom" id="productTabs" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="impuestos-tab" data-bs-toggle="tab" data-bs-target="#impuestos" type="button" role="tab" aria-controls="impuestos" aria-selected="false">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="vencimientos-tab" data-bs-toggle="tab" data-bs-target="#vencimientos" type="button" role="tab" aria-controls="vencimientos" aria-selected="false">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <button type="button" class="btn btn-primary" onclick="generarCodigoInterno()" title="Generar nuevo código">
                                        <i class="bi bi-arrow-clockwise"></i>
                                    </button>
                                </div>
                                <small class="text-muted">Se genera automáticamente (PROD-XXXXXX)</small>
                            </div>
    ...
    <script>
        ...
        function generarCodigoInterno() {
            // Obtener el último código del servidor
            fetch('../../obtener_ultimo_codigo.php')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        document.getElementById('codigo_interno').value = data.codigo;
                        ...
                        alert.innerHTML = `
                            <i class="bi bi-check-circle me-2"></i>
                            <strong>¡Código generado!</strong> ${data.codigo}
                            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                        `;
                        ...
                        // Código por defecto en caso de error
                        document.getElementById('codigo_interno').value = 'PROD-0000001';
                    }
                })
                .catch(error => {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Las pestañas funcionan automáticamente con Bootstrap 5
            // No necesitamos JavaScript personalizado para hacerlas flexibles

            // Generar código interno automáticamente si es nuevo producto
            <?php if (!$es_edicion): ?>
        ...
        // FUNCIÓN PARA SALTAR ENTRE PESTAÑAS CON ATAJOS DE TECLADO
        document.addEventListener('keydown', function(e) {
            if (e.ctrlKey) {
                ...
                if (e.key === 'ArrowRight' && currentActiveIndex < tabButtons.length - 1) {
                    e.preventDefault();
